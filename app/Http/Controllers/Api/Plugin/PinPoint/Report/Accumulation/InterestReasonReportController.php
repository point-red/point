...
class InterestReasonReportController extends Controller
{
    public function index(Request $request)
    {
        ...
        $result = SalesVisitationInterestReason::join(SalesVisitation::getTableName(), SalesVisitation::getTableName().'.id', '=', SalesVisitationInterestReason::getTableName().'.sales_visitation_id')
            ->join('forms', 'forms.id', '=', SalesVisitation::getTableName().'.form_id')
            ->select(SalesVisitationInterestReason::getTableName().'.name as name');

        foreach($queries as $key => $query) {
            $weekNum = $key + 1;
            $result = $result->leftJoinSub($query, 'week'.$weekNum, function ($join) use ($weekNum) {
                $join->on(SalesVisitationInterestReason::getTableName().'.name', '=', 'week'.$weekNum.'.name');
    ...
    private function getInterestReason($date, $j, $i)
    {
        ...
        $reasons = SalesVisitationInterestReason::join(SalesVisitation::getTableName(), SalesVisitation::getTableName().'.id', '=', SalesVisitationInterestReason::getTableName().'.sales_visitation_id')
            ->join('forms', 'forms.id', '=', SalesVisitation::getTableName().'.form_id')
            ->select(SalesVisitationInterestReason::getTableName().'.name as name')
    ...
    private function getTotalPerWeek($date, $j, $i)
    {
        ...
        $totalCount = SalesVisitationInterestReason::join(SalesVisitation::getTableName(), SalesVisitation::getTableName().'.id', '=', SalesVisitationInterestReason::getTableName().'.sales_visitation_id')
            ->join('forms', 'forms.id', '=', SalesVisitation::getTableName().'.form_id')
