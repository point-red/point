    ...
    private static function insertInventory($form, $purchaseOrder, $purchaseReceive)
    {
        ...
        foreach ($purchaseReceive->items as $item) {
            if ($item->quantity > 0) {
                ...
                InventoryHelper::increase($form->id, $purchaseReceive->warehouse_id, $item->item_id, $item->item->units[0]->id, $item->production_number, $item->expiry_date, $item->quantity, $price);
            }
        }
    }
}
